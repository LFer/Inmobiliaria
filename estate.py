...
class estate(osv.osv):
    ...
    def button_estate_match(self, cr, uid, ids, context=None, *args):
        ...
        caracteristicas = ['largo','garaje','ute','piscina'] # Lista de las caracteristicas por la que se va a machear
        excluyentes = ['ose']
        porcent = len(caracteristicas) + len(excluyentes)
        ...
        for unaOP in objOport:    # Recorro las oportunidades
            ...
            for p in caracteristicas:    # Recorro las caracteristicas de la lista
                ...
                if obj[p] == unaOP[p]:
                    machea += 1
            #machea = (machea*100) / porcent
            #unaOP['score'] = scoreappend.machea + append('%')   # Actualizar score        
            if (machea >= (( min_score / 0.2) + 1)):    # Si machea
